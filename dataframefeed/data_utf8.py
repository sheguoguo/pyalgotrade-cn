...
def save_data():
    ...
    _df_inuse.to_csv('../data/code_inuse.csv')

#从网络中更新数据,code 必须为str，dat中的为int
def refresh_data(_start_ ='2015-08-01',_end_ = ct._TODAY_):
    dat = pd.read_csv('../data/code.csv',index_col=0)
    inuse = pd.read_csv('../data/code_inuse.csv',index_col=0,parse_dates=[0])
    new_inuse = []
        ...
        try:
            ...
            if _data_ is not None and _data_.size != 0:
                if os.path.exists(filename):
                    _data_.to_csv(filename, mode='a', header=None)
                else:
                    _data_.to_csv(filename)
    ...
    _df_inuse.to_csv('../data/code_new_inuse.csv')
...
def read_data():      
    ...
    for code in dat['code'].values:
        ...
        print i,code
        try:
            df = pd.read_csv('../data/%s.csv'%code,index_col=0,parse_dates=[0])  #parse_dates直接转换数据类型，不用再重新狗再累   
            if df is not None:
                dic[code] = df
        except IOError: 
            pass    #不行的话还是continue
...
def get_universe():
    try:
        dat = pd.read_csv('../data/code_inuse.csv',index_col=0,parse_dates=[0])
    except Exception: 
        dat = ts.get_industry_classified()
...
def get_data(code):
    try:
        dat = pd.read_csv('../data/%s.csv'%code,index_col=0,parse_dates=[0])  #parse_dates直接转换数据类型，不用再重新狗再累 
    except Exception:
        dat = None
    return dat
def get_macd(df):
...
#save_data()
#refresh_data()
#df = pd.read_csv('../data/600848.csv',index_col=0,parse_dates=[0]) 
#da = get_macd(df) 
#plt_macd(df,da)
#_data_ = pd.read_csv('../data/600848.csv',index_col=0)  
#dic = read_data()
...
def temp2():
    ...
    for code in dat['code'].values:
        ...
        print i,code
        try:
            _data_ = pd.read_csv('../data/%s.csv'%code,index_col=0,parse_dates=[0])   #默认取3年，code为str，start无效的,start 和end若当天有数据则全都取
            if _data_ is not None:
                if _data_.index[0] in ct._start_range and _data_.index[-1] in ct._end_range:                          #筛选一次代码，使用头尾都包含的代码
                    inuse.append(code)
        except IOError: 
...
def temp():
    ...
    for code in dat['code'].values:
            ...
            #print i,code
            try:
                _data_ = pd.read_csv('../data/%s.csv'%code,index_col=0,parse_dates=[0])  #默认取3年，start 8-1包括
...
def change_type_to_yahoo():
    inuse = pd.read_csv('../data/code_inuse.csv',index_col=0,parse_dates=[0])               
    inuse.to_csv('../data_yahoo/code_inuse.csv')
    ...
    for code in inuse['code'].values:
        ...
        _data_.to_csv('../data_yahoo/%s.csv'%code,columns=['Open','High','Low','Close','Volume','Adj Close'],date_format="%Y-%m-%d")
        
def change_code_type_to_yahoo(code):
    re_columns ={'high':'High','low':'Low','open':'Open','close':'Close','volume':'Volume','price_change':'Adj Close'}  
    _data_ = pd.read_csv('../data/%s.csv'%code,index_col=0,parse_dates=[0])  #默认取3年，start 8-1包括
    _data_=_data_.rename(columns=re_columns)
    _data_.index.name = 'Date'
...
#SbigVolume()
#_data_ = pd.read_csv('../data/600848.csv',index_col=0,parse_dates=[0])   #默认取3年，code为str，start无效的,start 和end若当天有数据则全都取
#_data_.plot() 
